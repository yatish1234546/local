  ...
  <v-container fluid fill-height class="pa-2">
    <v-layout row wrap>
      <v-card color="grey lighten-4" width="100%">
        <v-toolbar flat>
          <span class="title secondary--text">MOTION</span>
        </v-toolbar>
        <v-divider></v-divider>
        <v-card-text>
          <div class="">
            <time-series-chart
              ...
              yAxisLabel=""
              :datasets="datasets"
              colorScheme="office.Apothecary6"
            ></time-series-chart>
          </div>
        </v-card-text>
      </v-card>
...
export default {
  ...
  methods: {
    ...
    generateData() {
      ...
      const data = [this.randomBar(currentDate, 2)];
  ...
  watch: {
    range(newRange, oldRange) {
      console.log('Regenerating Motion data');
