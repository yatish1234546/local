  ...
  <v-container fluid fill-height class="pa-2">
    <v-layout row wrap>
      <v-card color="grey lighten-4" width="100%">
        <v-toolbar flat>
          <span class="title secondary--text">TEMPERATURE</span>
        </v-toolbar>
        <v-divider></v-divider>
        <v-card-text>
          <div class="">
...
export default {
  name: 'widget-temperature-timeseries',
  components: {
    TimeSeriesChart,
  },
  data() {
  ...
  methods: {
    ...
    generateData() {
      ...
      const data = [this.randomBar(currentDate, 60)];
  ...
  watch: {
    range(newRange, oldRange) {
      console.log('Regenerating Heat data');
